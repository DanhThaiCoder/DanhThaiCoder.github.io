...
async function showAllUser() {
    try {
        ...
        if (!data.result || data.result.length === 0) {
            document.getElementById("listPro").innerHTML = `<tr><td colspan="11" class="text-center">Không có dữ liệu</td></tr>`;
            return;
        }
        data.result.forEach((item) => {
            const formattedDate = new Date(item.dateOfBirth).toLocaleDateString("vi-VN");
...
async function addUser() {
    ...
    const dateOfBirthInput = document.getElementById("addDob").value;
...
async function editUser(id) {
    try {
        ...
        document.getElementById("currentEditImg").src = `http://localhost:3000/images/${result.img}`;
...
async function updateUser() {
    ...
    const dateOfBirth = document.getElementById("editDob")
        .value;
